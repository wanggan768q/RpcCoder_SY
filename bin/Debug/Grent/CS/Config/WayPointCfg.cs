...
public class WayPointElement
{
	...
	public int face_rotate;      	//生成点朝向	用角度表示角色传送/复活的朝向，采用unity中生成点Y轴的旋转量 MOVE TO WAYPONIT接口会用这个路点的角度，填-1是不改变当前角度
...
public class WayPointTable
{
  ...
  public List<WayPointElement> GetAllElement(Predicate<WayPointElement> matchCB = null)
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine[1]!="spawn_map_id"){Ex.Logger.Log("WayPoint.csv中字段[spawn_map_id]位置不对应"); return false; }
		if(vecLine[2]!="spawn_x"){Ex.Logger.Log("WayPoint.csv中字段[spawn_x]位置不对应"); return false; }
		if(vecLine[3]!="spawn_y"){Ex.Logger.Log("WayPoint.csv中字段[spawn_y]位置不对应"); return false; }
		if(vecLine[4]!="spawn_z"){Ex.Logger.Log("WayPoint.csv中字段[spawn_z]位置不对应"); return false; }
		if(vecLine[5]!="face_rotate"){Ex.Logger.Log("WayPoint.csv中字段[face_rotate]位置不对应"); return false; }
		...
		if(vecLine[9]!="fly_height"){Ex.Logger.Log("WayPoint.csv中字段[fly_height]位置不对应"); return false; }
		if(vecLine[10]!="pet_start"){Ex.Logger.Log("WayPoint.csv中字段[pet_start]位置不对应"); return false; }
	...
	public bool LoadCsv(string strContent)
	{
		...
		while(true)
		{
			...
			member.face_rotate=Convert.ToInt32(vecLine[5]);
			...
			member.fly_height=Convert.ToSingle(vecLine[9]);
			member.pet_start=Convert.ToInt32(vecLine[10]);
