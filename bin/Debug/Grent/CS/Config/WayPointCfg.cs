...
public class WayPointTable
{
  ...
  public List<WayPointElement> GetAllElement(Predicate<WayPointElement> matchCB = null)
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine[5]!="waypoint_next_id"){Ex.Logger.Log("WayPoint.csv中字段[waypoint_next_id]位置不对应"); return false; }
		if(vecLine[6]!="waypoint_animation"){Ex.Logger.Log("WayPoint.csv中字段[waypoint_animation]位置不对应"); return false; }
		if(vecLine[7]!="waypoint_delay"){Ex.Logger.Log("WayPoint.csv中字段[waypoint_delay]位置不对应"); return false; }
	...
	public bool LoadCsv(string strContent)
	{
		...
		while(true)
		{
			...
			member.waypoint_next_id=Convert.ToInt32(vecLine[5]);
			member.waypoint_animation=vecLine[6];
			member.waypoint_delay=Convert.ToInt32(vecLine[7]);
