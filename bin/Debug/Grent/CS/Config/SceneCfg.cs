...
public class SceneElement
{
	...
	public string scene_resource;	//场景资源	资源路径
	...
	public int camera_type;      	//摄像机类型	0：强制2.5D 1：3D&2.5D
...
public class SceneTable
{
  ...
  public List<SceneElement> GetAllElement(Predicate<SceneElement> matchCB = null)
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine[1]!="comment"){Ex.Logger.Log("Scene.csv中字段[comment]位置不对应"); return false; }
		if(vecLine[2]!="name"){Ex.Logger.Log("Scene.csv中字段[name]位置不对应"); return false; }
		if(vecLine[3]!="description"){Ex.Logger.Log("Scene.csv中字段[description]位置不对应"); return false; }
		if(vecLine[4]!="background_pic"){Ex.Logger.Log("Scene.csv中字段[background_pic]位置不对应"); return false; }
		if(vecLine[5]!="loading_pic"){Ex.Logger.Log("Scene.csv中字段[loading_pic]位置不对应"); return false; }
		if(vecLine[6]!="thread_id"){Ex.Logger.Log("Scene.csv中字段[thread_id]位置不对应"); return false; }
		if(vecLine[7]!="scene_type"){Ex.Logger.Log("Scene.csv中字段[scene_type]位置不对应"); return false; }
		if(vecLine[8]!="valid_pos_x"){Ex.Logger.Log("Scene.csv中字段[valid_pos_x]位置不对应"); return false; }
		if(vecLine[9]!="valid_pos_z"){Ex.Logger.Log("Scene.csv中字段[valid_pos_z]位置不对应"); return false; }
		if(vecLine[10]!="scene_resource"){Ex.Logger.Log("Scene.csv中字段[scene_resource]位置不对应"); return false; }
		if(vecLine[11]!="required_level"){Ex.Logger.Log("Scene.csv中字段[required_level]位置不对应"); return false; }
		if(vecLine[12]!="required_score"){Ex.Logger.Log("Scene.csv中字段[required_score]位置不对应"); return false; }
		if(vecLine[13]!="pre_quest"){Ex.Logger.Log("Scene.csv中字段[pre_quest]位置不对应"); return false; }
		if(vecLine[14]!="teleport_id"){Ex.Logger.Log("Scene.csv中字段[teleport_id]位置不对应"); return false; }
		if(vecLine[15]!="camera_type"){Ex.Logger.Log("Scene.csv中字段[camera_type]位置不对应"); return false; }
		if(vecLine[16]!="pre_music"){Ex.Logger.Log("Scene.csv中字段[pre_music]位置不对应"); return false; }
		if(vecLine[17]!="loop_music"){Ex.Logger.Log("Scene.csv中字段[loop_music]位置不对应"); return false; }
	...
	public bool LoadCsv(string strContent)
	{
		...
		while(true)
		{
			...
			member.thread_id=Convert.ToInt32(vecLine[6]);
			member.scene_type=Convert.ToInt32(vecLine[7]);
			member.valid_pos_x=Convert.ToSingle(vecLine[8]);
			member.valid_pos_z=Convert.ToSingle(vecLine[9]);
			member.scene_resource=vecLine[10];
			member.required_level=Convert.ToInt32(vecLine[11]);
			member.required_score=Convert.ToSingle(vecLine[12]);
			member.pre_quest=Convert.ToInt32(vecLine[13]);
			member.teleport_id=Convert.ToInt32(vecLine[14]);
			member.camera_type=Convert.ToInt32(vecLine[15]);
