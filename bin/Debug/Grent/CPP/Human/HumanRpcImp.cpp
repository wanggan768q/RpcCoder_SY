...
int ModuleHuman::RpcMoveCheck( INT64 UserId, HumanRpcMoveCheckNotifyWraper& Notify )
{
	...
	//检测字段[角色ID]数据范围
	if( Notify.GetObjId()<-1 && Notify.GetObjId()!=0)
	{
		assert(false);
		return 0;
	}
	if( Notify.GetObjId()>10000000 && Notify.GetObjId()!=0)
	{
		assert(false);
		return 0;
	}
...
*                     低16位无效
********************************************************************************************/
int ModuleHuman::RpcTimeCheck( INT64 UserId, HumanRpcTimeCheckNotifyWraper& Notify )
{

	//检测字段[随机数据]数据范围
	if( Notify.GetData()<-1 && Notify.GetData()!=-1)
	{
		assert(false);
		return 0;
	}
	if( Notify.GetData()>10000000 && Notify.GetData()!=-1)
	{
		assert(false);
		return 0;
	}
...
* Return:         无
********************************************************************************************/
void ModuleHuman::SendToClientMotionFlags( INT64 UserId, HumanRpcMotionFlagsNotifyWraper& Notify )
{
	MsgStreamMgr::Instance().SendMsg( UserId, 0, Notify );
}
...
*                     低16位为操作返回值，获取方法GET_OPERATION_RET_CODE(ret)
********************************************************************************************/
static int ModuleHuman::RpcRush( CPlayer* pPlayer, CPacket* pPacket)
{
...
*                     低16位为操作返回值，获取方法GET_OPERATION_RET_CODE(ret)
********************************************************************************************/
static int ModuleHuman::RpcStare( CPlayer* pPlayer, CPacket* pPacket)
{
...
* Return:         无
********************************************************************************************/
void ModuleHuman::SendToClientChangePosition( INT64 UserId, HumanRpcChangePositionNotifyWraper& Notify )
{
	MsgStreamMgr::Instance().SendMsg( UserId, 0, Notify );
}
...
* Return:         无
********************************************************************************************/
void ModuleHuman::SendToClientAttrChange( INT64 UserId, HumanRpcAttrChangeNotifyWraper& Notify )
{
	MsgStreamMgr::Instance().SendMsg( UserId, 0, Notify );
}
...
*                     低16位为操作返回值，获取方法GET_OPERATION_RET_CODE(ret)
********************************************************************************************/
static int ModuleHuman::RpcTrusteeship( CPlayer* pPlayer, CPacket* pPacket)
{
...
*                     低16位为操作返回值，获取方法GET_OPERATION_RET_CODE(ret)
********************************************************************************************/
static int ModuleHuman::RpcChangePlayerName( CPlayer* pPlayer, CPacket* pPacket)
{
