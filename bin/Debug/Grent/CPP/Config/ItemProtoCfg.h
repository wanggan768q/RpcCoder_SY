...
struct ItemProtoElement
{
	...
	int transfer_id_1;           	//中转【刺客】id	进行道具中专时 对应的【刺客】道具id
	int transfer_id_2;           	//中转【游侠】id	进行道具中专时 对应的【游侠】道具id
	int transfer_id_3;           	//中转【魔法师】id	进行道具中专时 对应的【魔法师】道具id
	int transfer_id_4;           	//中转【圣骑士】id	进行道具中专时 对应的【圣骑士】道具id
	int transfer_id_5;           	//中转【神秘使】id	进行道具中专时 对应的【神秘使】道具id
...
public:
	...
	bool LoadJson(const std::string& jsonFile)
	{
		...
		BOOST_FOREACH(boost::property_tree::ptree::value_type& v, sms_array)
		{
			...
			member.transfer_id_1=p.get<int>("transfer_id_1");
			member.transfer_id_2=p.get<int>("transfer_id_2");
			member.transfer_id_3=p.get<int>("transfer_id_3");
			member.transfer_id_4=p.get<int>("transfer_id_4");
			member.transfer_id_5=p.get<int>("transfer_id_5");
	...
	bool LoadCsv(string strContent)
	{
		...
		if(vecLine[15]!="transfer_id_1"){printf_message("ItemProto.csv中字段[transfer_id_1]位置不对应");assert(false); return false; }
		if(vecLine[16]!="transfer_id_2"){printf_message("ItemProto.csv中字段[transfer_id_2]位置不对应");assert(false); return false; }
		if(vecLine[17]!="transfer_id_3"){printf_message("ItemProto.csv中字段[transfer_id_3]位置不对应");assert(false); return false; }
		if(vecLine[18]!="transfer_id_4"){printf_message("ItemProto.csv中字段[transfer_id_4]位置不对应");assert(false); return false; }
		if(vecLine[19]!="transfer_id_5"){printf_message("ItemProto.csv中字段[transfer_id_5]位置不对应");assert(false); return false; }
		if(vecLine[20]!="stack"){printf_message("ItemProto.csv中字段[stack]位置不对应");assert(false); return false; }
		if(vecLine[21]!="bind"){printf_message("ItemProto.csv中字段[bind]位置不对应");assert(false); return false; }
		if(vecLine[22]!="merge_item"){printf_message("ItemProto.csv中字段[merge_item]位置不对应");assert(false); return false; }
		if(vecLine[23]!="pickup_type"){printf_message("ItemProto.csv中字段[pickup_type]位置不对应");assert(false); return false; }
		if(vecLine[24]!="item_effect"){printf_message("ItemProto.csv中字段[item_effect]位置不对应");assert(false); return false; }
		...
		if(vecLine[27]!="batch_use"){printf_message("ItemProto.csv中字段[batch_use]位置不对应");assert(false); return false; }
		if(vecLine[28]!="use_when_dead"){printf_message("ItemProto.csv中字段[use_when_dead]位置不对应");assert(false); return false; }
		if(vecLine[29]!="timer_type"){printf_message("ItemProto.csv中字段[timer_type]位置不对应");assert(false); return false; }
		if(vecLine[30]!="life_time"){printf_message("ItemProto.csv中字段[life_time]位置不对应");assert(false); return false; }
		if(vecLine[31]!="expire_time"){printf_message("ItemProto.csv中字段[expire_time]位置不对应");assert(false); return false; }
		if(vecLine[32]!="cross_scene_destory"){printf_message("ItemProto.csv中字段[cross_scene_destory]位置不对应");assert(false); return false; }
		if(vecLine[33]!="store_type"){printf_message("ItemProto.csv中字段[store_type]位置不对应");assert(false); return false; }
		if(vecLine[34]!="tag"){printf_message("ItemProto.csv中字段[tag]位置不对应");assert(false); return false; }
		if(vecLine[35]!="pickup_notice"){printf_message("ItemProto.csv中字段[pickup_notice]位置不对应");assert(false); return false; }
		...
		while(true)
		{
			...
			member.transfer_id_1=(int)atoi(vecLine[15].c_str());
			member.transfer_id_2=(int)atoi(vecLine[16].c_str());
			member.transfer_id_3=(int)atoi(vecLine[17].c_str());
			member.transfer_id_4=(int)atoi(vecLine[18].c_str());
			member.transfer_id_5=(int)atoi(vecLine[19].c_str());
			member.stack=(int)atoi(vecLine[20].c_str());
			member.bind=(int)atoi(vecLine[21].c_str());
			member.merge_item=(int)atoi(vecLine[22].c_str());
			member.pickup_type=(int)atoi(vecLine[23].c_str());
			member.item_effect=vecLine[24];
			member.required_scene=(int)atoi(vecLine[25].c_str());
			member.use_destory=(int)atoi(vecLine[26].c_str());
			member.batch_use=(int)atoi(vecLine[27].c_str());
			member.use_when_dead=(int)atoi(vecLine[28].c_str());
			member.timer_type=(int)atoi(vecLine[29].c_str());
			member.life_time=(int)atoi(vecLine[30].c_str());
			member.expire_time=(int)atoi(vecLine[31].c_str());
			member.cross_scene_destory=(int)atoi(vecLine[32].c_str());
			member.store_type=(int)atoi(vecLine[33].c_str());
			member.tag=(int)atoi(vecLine[34].c_str());
			member.pickup_notice=(int)atoi(vecLine[35].c_str());
