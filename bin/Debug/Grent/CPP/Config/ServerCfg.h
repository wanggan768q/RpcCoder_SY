...
private:
	ServerTable(){}
	~ServerTable(){}
	typedef unordered_map<int, ServerElement> MapElementMap;
	MapElementMap	m_mapElements;
	vector<ServerElement>	m_vecAllElements;
	ServerElement m_emptyItem;
public:
	static ServerTable& Instance()
	{
		static ServerTable sInstance;
		return sInstance;
	}

	const ServerElement* GetElement(int key)
	{
		MapElementMap::iterator it = m_mapElements.find(key);
		if (it == m_mapElements.end())
		{
			AssertEx(false, std::string(std::string("ServerTable: ") + std::to_string(key)).c_str());
			return NULL;
		}
		return &it->second;
	}
	...
	vector<ServerElement>&	GetAllElement()
	{
	...
	{
		#ifdef CONFIG_JSON
		return LoadJson("Server.json");
		#else
		string strTableContent;
		if( LoadConfigContent("Server.csv", strTableContent ) )
			return LoadCsv( strTableContent );
		if( !LoadConfigContent("Server.bin", strTableContent ) )
		{
			printf_message("配置文件[Server.bin]未找到");
			assert(false);
			return false;
		}
	...
	bool LoadCsv(string strContent)
	{
		...
		if(vecLine.size() != 8)
		{
			printf_message("Server.csv中列数量与生成的代码不匹配!");
			assert(false);
			return false;
		}
		if(vecLine[0]!="id")
		{
			printf_message("Server.csv中字段[id]位置不对应");
			assert(false); 
			return false; 
		}
		if(vecLine[1]!="comment")
		{
			printf_message("Server.csv中字段[comment]位置不对应");
			assert(false); 
			return false; 
		}
		if(vecLine[2]!="region")
		{
			printf_message("Server.csv中字段[region]位置不对应");
			assert(false);
			return false; 
		}
		if(vecLine[3]!="name")
		{
			printf_message("Server.csv中字段[name]位置不对应");
			assert(false); 
			return false; 
		}
		if(vecLine[4]!="status")
		{
			printf_message("Server.csv中字段[status]位置不对应");
			assert(false); 
			return false; 
		}
		if(vecLine[5]!="promo")
		{
			printf_message("Server.csv中字段[promo]位置不对应");
			assert(false);
			return false;
		}
		if(vecLine[6]!="ip")
		{
			printf_message("Server.csv中字段[ip]位置不对应");
			assert(false);
			return false;
		}
		if(vecLine[7]!="port")
		{
			printf_message("Server.csv中字段[port]位置不对应");
			assert(false);
			return false;
		}
