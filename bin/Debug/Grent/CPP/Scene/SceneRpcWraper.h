...
//离开GameServer回应封装类
class SceneRpcLogOutGameReplyWraper
{
public:
	//构造函数
...
//离开GameServer请求封装类
class SceneRpcLogOutGameAskWraper
{
public:
	//构造函数
...
//前端通知后端剧情动画播放完毕请求封装类
class SceneRpcShowFilmFinishAskWraper
{
public:
	//构造函数
...
//被提出场景通知封装类
class SceneRpcKickOutGameNotifyWraper
{
public:
	//构造函数
	SceneRpcKickOutGameNotifyWraper()
	{
		...
		m_Kick_reason = AppendStringWraper();
		m_Reason_id = -1;
	...
	SceneRpcKickOutGameNotify ToPB() const
	{
		...
		*v.mutable_kick_reason()= m_Kick_reason.ToPB();
...
private:
	//原因
	AppendStringWraper m_Kick_reason;
public:
	void SetKick_reason( const AppendStringWraper& v)
	{
		m_Kick_reason=v;
	}
	AppendStringWraper GetKick_reason()
	{
		return m_Kick_reason;
	}
	AppendStringWraper GetKick_reason() const
	{
		return m_Kick_reason;
	}
private:
...
//同步场景分线繁忙状态通知封装类
class SceneRpcSyncSceneLineStatsNotifyWraper
{
public:
	//构造函数
...
//ChangeLine回应封装类
class SceneRpcChangeLineReplyWraper
{
public:
	//构造函数
...
//ChangeLine请求封装类
class SceneRpcChangeLineAskWraper
{
public:
	//构造函数
...
//ChangeZone回应封装类
class SceneRpcWantChangeZoneReplyWraper
{
public:
	//构造函数
	SceneRpcWantChangeZoneReplyWraper()
	{
	...
	//赋值构造函数
	SceneRpcWantChangeZoneReplyWraper(const SceneRpcWantChangeZoneReply& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcWantChangeZoneReply& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcWantChangeZoneReply ToPB() const
	{
		SceneRpcWantChangeZoneReply v;
		v.set_result( m_Result );
	...
	bool ParseFromArray(const void* data, int size)
	{
		SceneRpcWantChangeZoneReply pb;
		if(!pb.ParseFromArray(data,size)){return false;}
		Init(pb);
		return true;
	}
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcWantChangeZoneReply& v)
	{
		m_Result = v.result();
...
//ChangeZone请求封装类
class SceneRpcWantChangeZoneAskWraper
{
public:
	//构造函数
	SceneRpcWantChangeZoneAskWraper()
	{
	...
	//赋值构造函数
	SceneRpcWantChangeZoneAskWraper(const SceneRpcWantChangeZoneAsk& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcWantChangeZoneAsk& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcWantChangeZoneAsk ToPB() const
	{
		SceneRpcWantChangeZoneAsk v;
		v.set_zoneid( m_Zoneid );
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcWantChangeZoneAsk& v)
	{
		m_Zoneid = v.zoneid();
...
//返回源服回应封装类
class SceneRpcReturnSrcServerReplyWraper
{
public:
	//构造函数
	SceneRpcReturnSrcServerReplyWraper()
	{
	...
	//赋值构造函数
	SceneRpcReturnSrcServerReplyWraper(const SceneRpcReturnSrcServerReply& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcReturnSrcServerReply& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcReturnSrcServerReply ToPB() const
	{
		SceneRpcReturnSrcServerReply v;
		v.set_result( m_Result );
	...
	bool ParseFromArray(const void* data, int size)
	{
		SceneRpcReturnSrcServerReply pb;
		if(!pb.ParseFromArray(data,size)){return false;}
		Init(pb);
		return true;
	}
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcReturnSrcServerReply& v)
	{
		m_Result = v.result();
...
//返回源服请求封装类
class SceneRpcReturnSrcServerAskWraper
{
public:
	//构造函数
	SceneRpcReturnSrcServerAskWraper()
	{
		

	}
	//赋值构造函数
	SceneRpcReturnSrcServerAskWraper(const SceneRpcReturnSrcServerAsk& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcReturnSrcServerAsk& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcReturnSrcServerAsk ToPB() const
	{
		SceneRpcReturnSrcServerAsk v;
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcReturnSrcServerAsk& v)
	{
...
//天气变化通知封装类
class SceneRpcWeatherChangeNotifyWraper
{
public:
	//构造函数
	SceneRpcWeatherChangeNotifyWraper()
	{
	...
	//赋值构造函数
	SceneRpcWeatherChangeNotifyWraper(const SceneRpcWeatherChangeNotify& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcWeatherChangeNotify& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcWeatherChangeNotify ToPB() const
	{
		SceneRpcWeatherChangeNotify v;
		v.set_weatherid( m_WeatherId );
	...
	bool ParseFromArray(const void* data, int size)
	{
		SceneRpcWeatherChangeNotify pb;
		if(!pb.ParseFromArray(data,size)){return false;}
		Init(pb);
		return true;
	}
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcWeatherChangeNotify& v)
	{
...
//SpecObjPos请求封装类
class SceneRpcSpecObjPosAskWraper
{
public:
	//构造函数
...
//前端通知后端剧情动画播放完毕回应封装类
class SceneRpcShowFilmFinishReplyWraper
{
public:
	//构造函数
...
//昼夜变化通知封装类
class SceneRpcDayNightChangeNotifyWraper
{
public:
	//构造函数
	SceneRpcDayNightChangeNotifyWraper()
	{
	...
	//赋值构造函数
	SceneRpcDayNightChangeNotifyWraper(const SceneRpcDayNightChangeNotify& v){ Init(v); }
	//等号重载函数
	void operator = (const SceneRpcDayNightChangeNotify& v){ Init(v); }
 	//转化成Protobuffer类型函数
	SceneRpcDayNightChangeNotify ToPB() const
	{
		SceneRpcDayNightChangeNotify v;
		v.set_daystage( m_DayStage );
		v.set_remaining( m_Remaining );
	...
	bool ParseFromArray(const void* data, int size)
	{
		SceneRpcDayNightChangeNotify pb;
		if(!pb.ParseFromArray(data,size)){return false;}
		Init(pb);
		return true;
	}
...
private:
	//从Protobuffer类型初始化
	void Init(const SceneRpcDayNightChangeNotify& v)
	{
...
//SpecObjPos回应封装类
class SceneRpcSpecObjPosReplyWraper
{
public:
	//构造函数
...
class SceneRpcChangeSceneLeaveNotifyWraper
{
public:
	//构造函数
...
private:
	...
	string m_GameToken;
public:
	void SetGameToken( const string& v)
	{
		m_GameToken=v;
	}
	string GetGameToken()
...
//进入镜像场景通知封装类
class SceneRpcSEnterMirrorSceneNotifyWraper
{
public:
	//构造函数
...
//进入镜像回应封装类
class SceneRpcEnterMirrorReplyWraper
{
public:
	//构造函数
...
//退出镜像回应封装类
class SceneRpcExitMirrorSceneReplyWraper
{
public:
	//构造函数
...
//退出镜像请求封装类
class SceneRpcExitMirrorSceneAskWraper
{
public:
	//构造函数
...
//进入镜像请求封装类
class SceneRpcEnterMirrorAskWraper
{
public:
	//构造函数
...
template<> struct MessageIdT<SceneRpcDayNightChangeNotifyWraper>{ enum{ID=RPC_CODE_SCENE_DAYNIGHTCHANGE_NOTIFY};};
template<> struct MessageIdT<SceneRpcWeatherChangeNotifyWraper>{ enum{ID=RPC_CODE_SCENE_WEATHERCHANGE_NOTIFY};};
