...
public class ItemProtoElement
{
	...
	public li item_effect;       	//物品效果	如果该字段有内容，表示点击使用按扭不会调用server_script  数组，按照以下格式填写  类型|主键值|参数 类型枚举 1 = 打开界面 2 = 预留
	public string server_script_id;	//物品效果	更新中 不填表示无法使用
...
public class ItemProtoTable
{
  ...
  public List<ItemProtoElement> GetAllElement(Predicate<ItemProtoElement> matchCB = null)
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine[8]!="subtype_name"){Ex.Logger.Log("ItemProto.csv中字段[subtype_name]位置不对应"); return false; }
		if(vecLine[9]!="level"){Ex.Logger.Log("ItemProto.csv中字段[level]位置不对应"); return false; }
		if(vecLine[10]!="rank"){Ex.Logger.Log("ItemProto.csv中字段[rank]位置不对应"); return false; }
		if(vecLine[11]!="marquee"){Ex.Logger.Log("ItemProto.csv中字段[marquee]位置不对应"); return false; }
		...
		if(vecLine[23]!="server_script_id"){Ex.Logger.Log("ItemProto.csv中字段[server_script_id]位置不对应"); return false; }
		if(vecLine[24]!="effect_param"){Ex.Logger.Log("ItemProto.csv中字段[effect_param]位置不对应"); return false; }
		...
		if(vecLine[27]!="global_cd"){Ex.Logger.Log("ItemProto.csv中字段[global_cd]位置不对应"); return false; }
		if(vecLine[28]!="group"){Ex.Logger.Log("ItemProto.csv中字段[group]位置不对应"); return false; }
		if(vecLine[29]!="batch_use"){Ex.Logger.Log("ItemProto.csv中字段[batch_use]位置不对应"); return false; }
		if(vecLine[30]!="fast_use"){Ex.Logger.Log("ItemProto.csv中字段[fast_use]位置不对应"); return false; }
		if(vecLine[31]!="use_when_dead"){Ex.Logger.Log("ItemProto.csv中字段[use_when_dead]位置不对应"); return false; }
		if(vecLine[32]!="timer_type"){Ex.Logger.Log("ItemProto.csv中字段[timer_type]位置不对应"); return false; }
		if(vecLine[33]!="life_time"){Ex.Logger.Log("ItemProto.csv中字段[life_time]位置不对应"); return false; }
		if(vecLine[34]!="expire_time"){Ex.Logger.Log("ItemProto.csv中字段[expire_time]位置不对应"); return false; }
		if(vecLine[35]!="cross_scene_destory"){Ex.Logger.Log("ItemProto.csv中字段[cross_scene_destory]位置不对应"); return false; }
		if(vecLine[36]!="store_type"){Ex.Logger.Log("ItemProto.csv中字段[store_type]位置不对应"); return false; }
		if(vecLine[37]!="tag"){Ex.Logger.Log("ItemProto.csv中字段[tag]位置不对应"); return false; }
		if(vecLine[38]!="pickup_notice"){Ex.Logger.Log("ItemProto.csv中字段[pickup_notice]位置不对应"); return false; }
		if(vecLine[39]!="external_id"){Ex.Logger.Log("ItemProto.csv中字段[external_id]位置不对应"); return false; }
		if(vecLine[40]!="can_mix"){Ex.Logger.Log("ItemProto.csv中字段[can_mix]位置不对应"); return false; }
		if(vecLine[41]!="can_break"){Ex.Logger.Log("ItemProto.csv中字段[can_break]位置不对应"); return false; }
		if(vecLine[42]!="break_get"){Ex.Logger.Log("ItemProto.csv中字段[break_get]位置不对应"); return false; }
		if(vecLine[43]!="break_count"){Ex.Logger.Log("ItemProto.csv中字段[break_count]位置不对应"); return false; }
		if(vecLine[44]!="unlock_lv"){Ex.Logger.Log("ItemProto.csv中字段[unlock_lv]位置不对应"); return false; }
		if(vecLine[45]!="source_id"){Ex.Logger.Log("ItemProto.csv中字段[source_id]位置不对应"); return false; }
	...
	public bool LoadCsv(string strContent)
	{
		...
		while(true)
		{
			...
			member.subtype_name=Convert.ToInt32(vecLine[8]);
			member.level=Convert.ToInt32(vecLine[9]);
			member.rank=Convert.ToInt32(vecLine[10]);
			member.marquee=Convert.ToInt32(vecLine[11]);
			...
			member.effect_param=vecLine[24];
			member.required_scene=Convert.ToInt32(vecLine[25]);
			member.use_destory=Convert.ToInt32(vecLine[26]);
			member.global_cd=Convert.ToInt32(vecLine[27]);
			member.group=Convert.ToInt32(vecLine[28]);
			member.batch_use=Convert.ToInt32(vecLine[29]);
			member.fast_use=Convert.ToInt32(vecLine[30]);
			member.use_when_dead=Convert.ToInt32(vecLine[31]);
			member.timer_type=Convert.ToInt32(vecLine[32]);
			member.life_time=Convert.ToInt32(vecLine[33]);
			member.expire_time=Convert.ToInt32(vecLine[34]);
			member.cross_scene_destory=Convert.ToInt32(vecLine[35]);
			member.store_type=Convert.ToInt32(vecLine[36]);
			member.tag=Convert.ToInt32(vecLine[37]);
			member.pickup_notice=Convert.ToInt32(vecLine[38]);
			member.external_id=Convert.ToInt32(vecLine[39]);
			member.can_mix=Convert.ToInt32(vecLine[40]);
			member.can_break=Convert.ToInt32(vecLine[41]);
			member.break_get=vecLine[42];
			member.break_count=vecLine[43];
			member.unlock_lv=Convert.ToInt32(vecLine[44]);
			member.source_id=Convert.ToInt32(vecLine[45]);
